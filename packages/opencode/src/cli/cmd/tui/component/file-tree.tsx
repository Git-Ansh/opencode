...
const IGNORED = new Set([
  ".git", "node_modules", "__pycache__", ".next", "dist", ".cache",
  ".svn", ".hg", "vendor", "target", ".idea", ".vscode", ".DS_Store",
  "coverage", ".turbo", ".parcel-cache", ".nuxt", ".output",
])
...
export function FileTree(props: {
  ...
  const projectDir = () => {
    const d = sync.data.path.directory || process.cwd()
    return d.replace(/\\/g, "/")
  }
        ...
        <scrollbox flexGrow={1}>
          <For each={flatNodes()}>
            {(entry, i) => {
              ...
              const icon = entry.node.isDir
                ? expanded[entry.node.path] ? "\u25BC " : "\u25B6 "
                : "  "
