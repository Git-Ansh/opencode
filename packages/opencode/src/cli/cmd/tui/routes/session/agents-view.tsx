...
export function AgentsView(props: { sessionID: string; filterMode?: Accessor<FilterMode>; setFilterMode?: (m: FilterMode) => void; showList?: Accessor<boolean>; setShowList?: (v: boolean) => void }) {
  ...
  const activeCount = createMemo(() =>
    allAgents().filter(a => isBusy(a.id)).length
  )
    ...
    for (const msg of agentMsgs) {
      ...
      for (const p of msgParts) {
        if (p.type === "text" && !p.synthetic && p.text) {
          ...
          if (status === "running") {
            items.push({ type: "tool-running", toolName: title, content: title })
          } else if (status === "completed") {
            const output = (p.state?.output ?? "").slice(0, 200)
            items.push({ type: "tool-done", toolName: title, toolOutput: output, content: title })
          } else if (status === "error") {
            items.push({ type: "tool-error", content: `${title}: ${p.state?.error ?? "error"}` })
          }
        }
      }
    }
        ...
        >
          <text fg={theme.text}>
            <b>{activeCount() > 0
              ...
              : `${allAgents().length} completed`}</b>
          </text>
          <box flexDirection="row" gap={1}>
            <For each={AGENT_FILTERS}>
              {(f) => {
                const count = () => f.mode === "active" ? activeCount() : f.mode === "done" ? doneCount() : allAgents().length
                return (
                  <text
                      ...
                      return (
                        ...
                        >
                          <box flexDirection="row" justifyContent="space-between">
                            <box flexDirection="row" gap={1}>
                              <text fg={busy() ? theme.warning : theme.success}>
                                {"\u25CF"}
                              </text>
                ...
                {(agent) => (
                  <box flexGrow={1}>
                    <box
                      ...
                      gap={1}
                    >
                      <text fg={isBusy(agent().id) ? theme.warning : theme.success}>
                        {"\u25CF"}
                      </text>
                    ...
                    <scrollbox flexGrow={1} paddingLeft={1} ref={(r: any) => {
                      createEffect(() => {
                        ...
                        {(item) => (
                          <Switch>
                            <Match when={item.type === "text"}>
                              <box paddingBottom={1}>
                                <text fg={theme.text} wrapMode="word">{item.content}</text>
                              </box>
                            </Match>
                            <Match when={item.type === "thinking"}>
                              <box paddingBottom={1}>
                                <text fg={theme.textMuted} wrapMode="word"><i>{item.content}</i></text>
                              </box>
                            </Match>
                            <Match when={item.type === "tool-running"}>
                              <box flexDirection="row" gap={1} paddingBottom={1}>
                                <text fg={theme.warning}>{"\u25CF"}</text>
                                <text fg={theme.warning} wrapMode="none">{item.toolName}</text>
                              </box>
                            </Match>
                            <Match when={item.type === "tool-done"}>
                              <box paddingBottom={1}>
                                <box flexDirection="row" gap={1}>
                                  ...
                                  <text fg={theme.text} wrapMode="none">{item.toolName}</text>
                                </box>
                                <Show when={item.toolOutput}>
                                  <box paddingLeft={2}>
                                    <text fg={theme.textMuted} wrapMode="word">{item.toolOutput}</text>
                                  </box>
                                </Show>
                              </box>
                            </Match>
                            <Match when={item.type === "tool-error"}>
                              <box flexDirection="row" gap={1} paddingBottom={1}>
                                ...
                                <text fg={theme.error} wrapMode="word">{item.content}</text>
                              </box>
                            </Match>
                          </Switch>
                        )}
