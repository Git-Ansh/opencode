...
import { openFileInEditor } from "./file-preview"
import path from "path"
import { watch, type FSWatcher } from "node:fs"

interface FileEntry {
  file: string
  // Unified diff patch, when known up-front (from session_diff or a tool's metadata).
  patch?: string
  additions: number
  deletions: number
  status?: string
...
export function DiffView(props: {
  ...
  const rawFiles = createMemo((): FileEntry[] => {
    ...
    // Modified files from session_diff (server already computed a unified patch)
      ...
      for (const msg of msgs) {
        ...
        for (const p of parts) {
          ...
          if (p.tool === "write" || p.tool === "edit" || p.tool === "apply_patch") {
            const fp = (p.state.input as { filePath?: string }).filePath ?? ""
            if (fp && (p.state.status === "completed" || p.state.status === "running")) {
              const metadata =
                p.state.status === "completed" || p.state.status === "running" ? p.state.metadata : undefined
              toolFiles.set(fp, { tool: p.tool, diff: (metadata as { diff?: string } | undefined)?.diff })
            }
          }
        }
      }
    ...
    for (const msg of msgs) {
      ...
      for (const p of parts) {
        if (p.type === "tool" && p.tool === "read" && p.state.status === "completed") {
          ...
          const fp = input.filePath ?? input.file_path ?? p.state.title ?? ""
          if (fp && !seen.has(fp)) {
            result.push({
              file: fp,
              additions: 0,
  ...
  // Load file content for preview (used when there's no patch to display)
  ...
  const diffContent = createMemo(() => current()?.patch ?? "")
  ...
  useKeyboard((evt) => {
    ...
    if (evt.ctrl && evt.name === "e") {
      evt.preventDefault()
      const file = current()
      if (file) void openFileInEditor(renderer, file.file)
    }
      ...
      >
        ...
        <box flexDirection="row" gap={1}>
          <For each={FILE_FILTERS}>
            {(f) => {
              const count = () =>
                f.mode === "modified" ? modifiedCount() : f.mode === "opened" ? openedCount() : validFiles().length
              return (
                <text
                    ...
                    return (
                      <>
                        <Show when={showSeparator()}>
                          <text fg={theme.border} paddingLeft={1}>
                            {"─── opened ───"}
                          </text>
                        </Show>
                        <box
                  ...
                  <Show when={!diffContent() && fileContent()}>
                    <scrollbox flexGrow={1} paddingLeft={1}>
                      <For each={fileContent().split("\n")}>
                        {(line, i) => (
                          <text fg={theme.text} wrapMode="none">
                            <span style={{ fg: theme.diffLineNumber }}>{String(i() + 1).padStart(4)} </span>
                            {line}
                          </text>
                        )}
                      </For>
