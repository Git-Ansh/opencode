...
export function PlanReview(props: {
  ...
  useKeyboard((evt) => {
    ...
    if (evt.shift && !evt.ctrl && !evt.option) {
    ...
    // command-keybind handler which fires before this and ignores
    // defaultPrevented. Submission is click-driven / F3 instead —
    // Ctrl+S is XOFF on Windows.
    if (shouldDefer()) return
      ...
      <box paddingLeft={1} paddingRight={1} paddingTop={1}>
        ...
        <box flexDirection="row" gap={2} paddingTop={1} flexWrap="wrap" flexShrink={0}>
          <box onMouseDown={() => acceptAllPending()}>
            <text fg={pendingCount() > 0 ? theme.accent : theme.textMuted}>[ Accept all pending ]</text>
          </box>
          <box onMouseDown={() => rejectAll()}>
            <text fg={store.steps.length > 0 ? theme.error : theme.textMuted}>[ Reject all ]</text>
          </box>
                ...
                >
                  ...
                  <Show when={step.files && step.files.length > 0}>
                    <box paddingLeft={3} flexDirection="row" gap={1} flexWrap="wrap">
                      <For each={step.files}>{(file) => <text fg={theme.secondary}>{file}</text>}</For>
                    </box>
                  </Show>
                  ...
                  <Show when={isActive() && !commenting()}>
                    ...
                    >
                      <text fg={theme.accent}>[ {step.comment ? "edit" : "add"} comment ]</text>
                    </box>
                  </Show>
                  <Show when={isActive() && commenting()}>
