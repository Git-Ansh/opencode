...
// Tool state varies by status (pending/running/completed/error) — narrow before
// reading status-specific fields like output/error/time/metadata.
function toProcessEntry(part: ToolPart): ProcessEntry {
  const state = part.state
  const input = state.input as { command?: string } | undefined
  const command = input?.command ?? "unknown"
  const output = state.status === "completed" ? state.output : state.status === "error" ? state.error : ""
  const startTime = state.status === "pending" ? 0 : state.time.start
  const endTime = state.status === "completed" || state.status === "error" ? state.time.end : undefined
  const exitCode =
    state.status === "completed" ? ((state.metadata as { exit?: number } | undefined)?.exit ?? undefined) : undefined

  return {
    id: part.id,
    command,
    state: state.status === "running" ? "running" : state.status === "error" ? "error" : "completed",
    output: output ?? "",
    startTime,
    endTime,
    exitCode,
...
export function TerminalView(props: {
  ...
  const allProcesses = createMemo(() => {
    ...
    for (const part of parts) {
      ...
      procs.push(toProcessEntry(part))
    }
      ...
      >
        ...
        <box flexDirection="row" gap={1}>
          <For each={TERMINAL_FILTERS}>
            {(f) => {
              const count = () =>
                f.mode === "running" ? runningCount() : f.mode === "done" ? doneCount() : allProcesses().length
              return (
                <text
                  ...
                  >
                    ...
                    <Show when={proc().exitCode !== undefined}>
                      <text fg={proc().exitCode === 0 ? theme.success : theme.error}>
                        {proc().exitCode === 0 ? "─── exited (0)" : `─── exited (${proc().exitCode})`}
                      </text>
                    </Show>
                  </scrollbox>
                </box>
